...
# 恢复 guardrails hook（worktree only，不 stage 以免污染 squash commit）
if git restore --source="$OLD_BRANCH" --worktree -- .claude/hooks/block-dangerous-git.sh 2>/dev/null; then
