...
class TestCostCircuitBreakerRecord:
    ...
    def test_updates_existing_row_same_module_key(self, temp_db_path, settings):
        from alphascreener.core.cost import CostCircuitBreaker

        cb = CostCircuitBreaker(settings)
        cb.record(date.today(), 0.50, 5, '{"screening": 0.50}')
        cb.record(date.today(), 0.30, 3, '{"screening": 0.30}')
        ...
        assert row[0] == 0.80
        assert row[1] == 8
        assert json.loads(row[2]) == {"screening": 0.80}
